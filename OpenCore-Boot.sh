...
args=(
  -m "$ALLOCATED_RAM" -cpu Penryn,kvm=on,vendor=GenuineIntel,+invtsc,vmware-cpuid-freq=on,"$MY_OPTIONS"
  -machine q35
  ...
  -drive id=InstallMedia,if=none,file="$REPO_PATH/mac.img",format=raw
  -device ide-hd,bus=sata.2,drive=OpenCoreBoot
  -device ide-hd,bus=sata.3,drive=InstallMedia
  #-drive id=MacHDD,if=none,file="$REPO_PATH/mac.img",format=qcow2
